      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: _SoftButton(icon: Icons.folder_open, label: 'Ver expediente completo')),
              const SizedBox(width: 10),
              Expanded(child: _SoftButton(icon: Icons.local_hospital_outlined, label: 'Llamar Doctor EMERGENCIA')),
            ]),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
            Text('Isela', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
            Text('Altea',
                style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF0078B3))),
          ]),
        ]),
      ),
      const Spacer(),
      Column(children: const [
        Text('Riesgo estimado', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500)),
        SizedBox(height: 6),
...
class _SoftButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFFE1F0F7), Color(0xFFFDFEFF)]),
        borderRadius: BorderRadius.circular(20),
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Flexible(child: Text(label, style: const TextStyle(fontSize: 11), maxLines: 2)),
      ]),
    );
  }
}
...
class RiskGauge extends StatelessWidget {
  ...
  const RiskGauge({super.key, required this.percent, required this.label, required this.value});
      ...
      child: CustomPaint(
        ...
        child: Center(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Text(label, style: const TextStyle(fontSize: 10, color: Color(0xFF759F7A))),
            Text(value,
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: center, radius: radius), pi * 0.75, pi * 1.5, false, paint);

    paint.color = AppColors.green;
    canvas.drawArc(
        Rect.fromCircle(center: center, radius: radius), pi * 0.75, pi * 1.5 * percent, false, paint);
  }
