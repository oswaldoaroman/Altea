...
class _InformesScreenState extends State<InformesScreen> with SingleTickerProviderStateMixin {
  ...
  static const _reportes = ['Chequeo cardiovascular', 'Perfil de lípidos', 'Evaluación de riesgo'];
  ...
  Widget build(BuildContext context) {
    return ResponsiveBody(
      child: Column(
        ...
        children: [
          const TopTitle(title: 'Informes y servicios', subtitle: 'Todo tu historial en un lugar'),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 18),
            child: Container(
              decoration: BoxDecoration(color: AppColors.sky, borderRadius: BorderRadius.circular(16)),
              ...
              child: TabBar(
                ...
                tabs: const [Tab(text: 'Informes'), Tab(text: 'Voz'), Tab(text: 'Médicos')],
              ),
            ),
          ),
          Expanded(
            child: TabBarView(
              controller: _tab,
              children: [
                _ListaSimple(items: _reportes, icon: Icons.description_rounded, actionLabel: 'Generar informe PDF completo'),
                _ListaSimple(items: _registros, icon: Icons.mic_rounded, actionLabel: 'Grabar nuevo registro', actionColor: AppColors.teal),
...
class _ListaSimple extends StatelessWidget {
  ...
  const _ListaSimple({required this.items, required this.icon, required this.actionLabel, this.actionColor = AppColors.navy});
      ...
      children: medicos.map((m) {
        ...
        return Padding(
          ...
          child: AppCard(
            child: Row(
              children: [
                const CircleAvatar(backgroundColor: AppColors.navy, child: Icon(Icons.medical_services_rounded, color: Colors.white, size: 16)),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(nombre, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12.5)),
                      Text(detalle, style: const TextStyle(fontSize: 11, color: AppColors.slate)),
                    ],
                  ),
                ),
                OutlinedButton(
                  ...
                  child: const Text('Agendar', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800)),
                ),
              ],
            ),
          ),
